...
- **事実で答える**: メインエージェントは、ファイルの内容・コマンドの結果を述べる前に、そのファイルを Read するかコマンドを実行する ／ 例: 「テストは通っている」と書く前に `npm test` を実行する ／ 失効: なし
- **ルール追加の窓口**: メインエージェントは、新しい規範を思いついたら `_meta.md` のパイプラインを通す ／ 例: `/add-rule "完了 commit に台帳を含める"` ／ 失効: なし
- **作業の起点は台帳**: メインエージェントは、作業を始める前に `docs/tasks/list.md` を Read し、着手する task の status を 進行中 に更新する ／ 例: `/start-task 3` ／ 失効: 台帳を廃止したとき
