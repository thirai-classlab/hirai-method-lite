...
## 5. 第三者マーケットプレイスは既定で自動更新されない

**何が起きたか**: marketplace 側のプラグインを更新しても、インストール済みの環境には反映されなかった。

**なぜ起きたか**: Claude Code はマーケットプレイス単位の自動更新を持っているが、**第三者・ローカル開発のマーケットプレイスは既定で無効**（Anthropic 公式のものだけ既定で有効）。無効の間は更新検知が **通知するだけ**で、適用は手動の 2 手順を要する。
...
**どう対処するか**: プラグインを配布側として直したら、利用側に「上の 2 コマンドを叩く必要がある」ことを明示的に伝える。直したことと届いたことは別。自動化したい利用者には、**自前で実装せず** `/plugin` → Marketplaces → **Enable auto-update**（または `settings.json` の `extraKnownMarketplaces.<name>.autoUpdate: true`）を案内する。有効にすると、Claude Code が起動後の背景で（最大 10 分の遅延を挟んで）一覧と本体を更新し、反映は `/reload-plugins` か次回起動から。

**実測 (2026-09-03, Claude Code 2.1.258)**: `claude plugin marketplace update <name>`（約 1.8 秒）と `claude plugin update <plugin>@<marketplace> --scope user`（約 0.4 秒）は、**動いている Claude Code セッションの内側から呼んでも成功する**（ロック競合・待ちは起きなかった。1.9.0 → 1.12.0 を確認）。非対話でも `-y` は不要（`command` ソースのプラグインだけが要求される）。ただし **hook から叩くのは二重実装**であり、セッション開始を通信で待たせるので採らない。

**教訓の一般形**: 「本体に無い」と決める前に、**本体の設定を先に探す**。あるなら案内するのが正解で、自前実装は車輪の再発明になる。

**出典**: 本セッション実測 (2026-08-27) + 自動更新の追記 (2026-09-03)。
